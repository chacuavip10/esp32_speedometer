...
class UbxNavPvtSimulator:
    def __init__(
        ...
        print("\n" + "=" * 72)
        print(f"[CONNECTED] Port: {serial_port}  Baud: {baudrate}  Rate: {self.send_hz:.3g} Hz")
        print(f"[MESSAGES ] " + self._format_msgs_line_config_only())
        ...
        print("Profiles cycle includes LOST profiles unless disabled via --disable-gps-lost")
    ...
    def build_nav_pvt(self) -> bytes:
        ...
        payload += struct.pack('<l', int((self.height_m * 1000.0) + math.sin(nowt) * 5000))
        payload += struct.pack('<l', int((self.height_m * 1000.0) - 2000 + math.sin(nowt) * 5000))
    ...
    def build_nav_sol(self) -> bytes:
        ...
        x_m, y_m, z_m = geodetic_to_ecef(self.current_lat, self.current_lon, self.height_m)
        ...
        vd = 0.0
        vx_mps, vy_mps, vz_mps = ned_to_ecef(self.current_lat, self.current_lon, vn, ve, vd)
    ...
    def _profiles_cycle(self):
        base = ['STATIONARY', 'WALKING', 'CITY_DRIVING', 'HIGHWAY_DRIVING', 'HIGH_SPEED']
    ...
    def update_simulation(self, now_pc: float):
        ...
        if now_pc - self.profile_change_time > self.profile_duration:
            ...
            self.gps_lost_start = now_pc if self.current_profile in ('GPS_LOST', 'GPS_LOST_SILENT') else 0.0
            ...
            if self.current_profile == 'GPS_LOST':
                ...
                if profile_info['min'] == profile_info['max']:
                    ...
                    print(f"   Speed: {profile_info['min']} → {profile_info['max']} km/h (linear ramp over 15s)")
            print("=" * 72)

        # LOST windows (skip entirely if disabled)
        if self.gps_lost_enabled and self.current_profile in ('GPS_LOST', 'GPS_LOST_SILENT'):
        ...
        if target_speed >= 0:
            ...
            if abs(speed_diff) > 0.1:
                ...
                acceleration_rate = 0.1  # Adjust this to control how quickly speed changes
            ...
            self.simulated_heading = (self.simulated_heading + random.uniform(-1, 1)) % 360
            
            # Update position if moving
            if self.simulated_speed > 0.1:
                # Scale position change based on speed
                position_factor = self.simulated_speed / 100.0  # Normalize to reasonable movement
        ...
        # iTOW ms counter
        self.time_of_week = (self.time_of_week + int(self.send_interval * 1000)) % 604_800_000
    ...
    def print_status(self, now_pc: float):
        ...
        if self.gps_lost_enabled and self.current_profile == 'GPS_LOST_SILENT':
            ...
            print(f"[STATUS] {profile_display}   (GPS_LOST={'ON' if self.gps_lost_enabled else 'OFF'})")
            print(f"   Messages:  {msgs_line}")
            
            if profile_info['min'] == profile_info['max']:
                print(f"   Speed:   {self.simulated_speed:.1f} km/h (constant)")
            else:
                print(f"   Speed:   {self.simulated_speed:.1f} km/h (target: {target_speed:.1f}, range: {profile_info['min']}-{profile_info['max']})")
...
def main():
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python ubx_simulator.py PORT [--rate float] [--baudrate int] [--enable list] [--disable list] [--disable-gps-lost]")
